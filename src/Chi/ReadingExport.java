...
public class ReadingExport {
	...
	private static SimpleDateFormat HTMLDateFormat = new SimpleDateFormat("dd MMMM YYYY, KK:mm:ss");
	
	public static void export (ArrayList<Row> rows) {
		TimeSeriesCollection dataset=new TimeSeriesCollection();
		TimeSeries s1=new TimeSeries("Usage");
		for (Row row : rows) {
			s1.addOrUpdate(new Second(row.getTimestamp(2)),row.getDouble(3));
		}
		...
		for (int i=0;i<rows.size();i++) {
			sb.append("<tr>");
			sb.append("<td>");
			sb.append(HTMLDateFormat.format(rows.get(i).getTimestamp(2)));
			sb.append("</td>");
			sb.append("<td>");
			sb.append(rows.get(i).getDouble(3));
			sb.append("</td>");
			sb.append("</tr>");
		}
		sb.append("</tbody></table><p class=\"Foot\">Generated by ");
		sb.append(Config.APP_NAME+" at "+HTMLDateFormat.format(new Date())+".");
		sb.append("</p>");
		sb.append("</body>");
		sb.append("</html>");
